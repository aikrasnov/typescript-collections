...
describe('selection sort', () => {
    it('should sort array', () => {
        const sorted = index_1.selectionSort(array_for_sort_1.unsorted);
        for (let i = 0; i < sorted.length - 1; i++) {
            expect(sorted[i].compareTo(sorted[i + 1])).toStrictEqual(-1);
        }
    });
    it('should sort array in reverse order', () => {
        const sorted = index_1.selectionSort(array_for_sort_1.unsorted, true);
        for (let i = 0; i < sorted.length - 1; i++) {
            expect(sorted[i].compareTo(sorted[i + 1])).toStrictEqual(1);
        }
    });
    it('should sort empty array without error', () => {
        index_1.selectionSort(array_for_sort_1.empty);
    });
    it('should sort array of equal values', () => __awaiter(this, void 0, void 0, function* () {
        const sorted = index_1.selectionSort(array_for_sort_1.equalValue);
        for (let i = 0; i < sorted.length - 1; i++) {
